...
If DatabaseQuery(#Database, Query$)                                   ; Build the table if you can
  While NextDatabaseRow(#Database)
  
    record$ = GetDatabaseString(#Database, 0)
    dosname$ = GetDatabaseString(#Database, 1)
    filename$ = GetDatabaseString(#Database, 2)
    filetype$ = GetDatabaseString(#Database, 3)
    category$ = GetDatabaseString(#Database, 4)
    collection$ = GetDatabaseString(#Database, 5)
    display$ = GetDatabaseString(#Database, 6)
    trademark$ = GetDatabaseString(#Database, 7)
    trademarklink$ = GetDatabaseString(#Database, 8)
    filesize$ = GetDatabaseString(#Database, 9)
...
If OpenDatabase(#Database, "IncrediMail", "", "") ; I've alreadyd efined, filled and connected my database :)
...
If hWnd = 0 Or CreateGadgetList(WindowID(0)) = 0
