...
Procedure GetTextDatas(text.s)
  ...
  For x=TextX To TextX+TextWidth(text)
    For y=TextY To 599
      If Point(x,y)<>$000000
        AddElement(pixel())
        pixel()\xz\z1=Random(799)    ;ausweichziel..
        pixel()\yz\z1=Random(599)    ;<<
        pixel()\xz\z2=x  ;endziel..
        pixel()\yz\z2=y  ;<<
...
Procedure farbverlauf()
  flife-1
  If flife<0
    bw=Random(32)-9/10        ; geändert von bw=(Random(18)-9)/10  in bw=Random(32)-9/10  by nicolaus
    gw=Random(32)-9/10        ; geändert von gw=(Random(18)-9)/10  in gw=Random(32)-9/10  by nicolaus
    rw=Random(32)-9/10        ; geändert von rw=(Random(18)-9)/10  in rw=Random(32)-9/10  by nicolaus
    flife=Random(90)+20
  EndIf
  ...
  Else
    bw*-1
    blau = 90       ; hinzugefügt by nicolaus
  EndIf
  If gruen+gw<255 And gruen+gw>60   ; von 90 auf 60 geändert  by nicolaus
    gruen+gw
  Else
    gw*-1
    gruen = 60      ; hinzugefüg  by nicolaus
  EndIf
  If rot+rw<255 And rot+rw>60       ; von 90 auf 60 geändert
    rot+rw
  Else
    rw*-1
    rot = 60        ; hinzugefügt  by nicolaus
  EndIf
EndProcedure
